...
async function seed() {
  ...
  const Experience = mongoose.model('Experience', new mongoose.Schema({ exp_id: String }, { strict: false }));
  const Project = mongoose.model('Project', new mongoose.Schema({ project_id: String }, { strict: false }));
  const Skill = mongoose.model('Skill', new mongoose.Schema({ category: String }, { strict: false }));

  console.log('Seeding Experiences...');
  for (const exp of experiences) {
    await Experience.updateOne({ exp_id: exp.exp_id }, { $set: exp }, { upsert: true });
  }

  console.log('Seeding Projects...');
  for (const proj of projects) {
    await Project.updateOne({ project_id: proj.project_id }, { $set: proj }, { upsert: true });
  }

  console.log('Seeding Skills...');
  for (const skill of skills) {
    await Skill.updateOne({ category: skill.category }, { $set: skill }, { upsert: true });
  }
